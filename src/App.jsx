...
function AIScanner({ onConfirm, onCancel }) {
  ...
  const processFile = useCallback(async (file) => {
    ...
    try {
      ...
      catch { throw new Error("throw new Error("throw new Error("Could not read document. Try a clearer image."););"); }
        ...
        <div className="card">
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"13px"}}>
            ...
            <div style={{background:"var(--brand-dim)",borderRadius:"7px",padding:"10px 13px",fontSize:"12px",color:"var(--brand)",lineHeight:1.7}}>
              💡 Next BAS due: <strong>28 October 2025</strong> (Q1 FY26)
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="inv-wrap">
          <div className="inv-hd">
            <div>
              ...
              <div style={{fontSize:"11.5px",color:"#888"}}>ABN {abn}</div>
            </div>
...
function Expenses({ expenses, setExpenses, onSave, onDelete }) {
  ...
  const totAmt = expenses.reduce((a,e)=>a+e.amount,0);
  const totGST = expenses.filter(e=>e.gstIncluded).reduce((a,e)=>a+e.amount/11,0);
  ...
  return (
    <div>
      ...
      <div className="g3" style={{marginBottom:"18px"}}>
        <div className="card card-xs"><div className="sl">Total Expenses</div><div className="sv">{fmt(totAmt)}</div><div className="ss">Incl. GST</div></div>
        <div className="card card-xs"><div className="sl">Excl. GST</div><div className="sv">{fmt(totAmt-totGST)}</div><div className="ss">Deductible base</div></div>
